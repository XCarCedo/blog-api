...
from dependencies.auth import SuperUserDep, UserDep
from dependencies.database import SessionDep
from schemas.blogs import BlogCreate, BlogPublic, BlogUpdate
...
async def get_blog_by_id(user: UserDep, blog_id: int, session: SessionDep):
    ...
    if not blog:
        raise HTTPException(
            ...
            f"Couldn't find a Blog with id of {str(blog_id)!r}",
        )
...
async def update_blog(
    user: SuperUserDep, blog_id: int, session: SessionDep, blog_update: BlogUpdate
):
